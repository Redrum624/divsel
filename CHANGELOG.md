...
First release. Nothing preceded it, so every entry below is an addition. Where a
shipped artifact has pre-release history worth knowing — the conformance
contract was revised once, before release, after an independent differential —
that history is recorded inside the entry that ships it.
...
- **Golden fixtures** (`test-assets/golden-selection.json`, schema 1): 22 cases —
  ...
  10, 18, 22; see `docs/CONFORMANCE.md`). Reproduced by both the Rust
  (`cargo test --test golden`) and Python (`pytest python/tests/test_golden.py`)
  readers, and regenerated byte-identically by `python/tools/gen_golden.py --check`.
- **Cross-implementation conformance contract** (`docs/CONFORMANCE.md`), in the
  form an independent 90,000-instance differential left it. Aura's pure-Python
  port — written from that document alone — was run against this implementation
  on 2026-08-26 with 0 algorithmic disagreements. The differential changed **no
  library behaviour**, and `test-assets/golden-selection.json` is byte-identical
  to the fixture that predates it; what it changed is the contract a port is
  held to.
  - The float tolerance for `expected_f` is **derived from the primitives**:
    `tol(expected_g) + lam * tol(expected_div)` with
    `tol(x) = f_rel * max(1, abs(x))`, rather than a flat `tol(expected_f)`. `f`
    is `g + lam * div`, so a distance ulp reaches `f` multiplied by `lam`, and
    the flat rule failed a correct port at high `lam` (147 of the 90,000
    instances). The other float fields keep the `tol(expected)` rule, and the
    ...
    the derived rule and pin it with a regression test.
  - The document states what the `f` check does and does not detect, measured
    rather than asserted — a stronger claim drafted alongside the tolerance fix
    ("every one of those is gross, not marginal") was disproved by measurement
    and removed before release. The `f` check is frequently the **sole**
    ...
    of budget where the flat bound rejected it at 3.81x, and the whole contract
    then passes. The section gives the escape band
    (`div ∈ (1.6e-08, 5.2e-07)` at `lam = 64`, 21.7% of 60,000 exact-duplicate
    draws), records that no committed fixture is anywhere near it, and argues
...
- **`expected_threshold` is a selected grid entry** section in
  ...
  the derived tolerance leaves on the flat bound. Because `threshold` is a
  *choice* among grid entries and not a measured quantity, its error is
  quantized to a factor `1 + eps` (9.09% at the default `eps = 0.1`) — a correct
  float64 port reproduces all 22 to within `3.35e-08` relative, 3.4% of the
  bound, while a neighbouring entry is 90,909x the bound away — so no tolerance
  can separate a false failure from a real one. The rule is therefore
  **deliberately left flat**: a port sweeping the wrong grid is caught on 8 of
  the 22 through this field alone (one entry too few: cases 1, 3, 4, 5, 7, 9,
  10, 18), and both candidate relaxations were rejected on measurement. The mode
  is documented instead, the way Finding B is, with the provoking families and
  the diagnostic a harness runs. Rust test
...
- **Degenerate geometry** section in `docs/CONFORMANCE.md`: on exact duplicates,
  ...
  *less* accurate side, so a float64 port legitimately disagrees about
  ...
  does not cover this and says so.
